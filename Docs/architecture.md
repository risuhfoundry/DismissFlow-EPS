...
Because the application and backend are now committed, sections below cite the
actual file paths in the repository. Anything still `Planned:` is genuinely
absent and is called out for follow-up work.
...
> **Implementation note:** As of 2026-08-27 the schema, RLS, Edge Functions, and all four portals are committed and wired up. `Implemented` in the conventions table means the component is in the repository; `Planned` means it remains genuinely absent and is called out in §20.2. The `Future` items in §20.3 are post-prototype scope.
